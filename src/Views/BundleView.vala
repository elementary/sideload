...
public class Sideload.BundleView : AbstractView {
    ...
    private Gtk.Label install_size_label;
    ...
    construct {
        ...
        var install_size_icon = new Gtk.Image.from_icon_name ("browser-download-symbolic", Gtk.IconSize.BUTTON);
        install_size_icon.valign = Gtk.Align.START;

        unowned Gtk.StyleContext install_context = install_size_icon.get_style_context ();
        install_context.add_class (Granite.STYLE_CLASS_ACCENT);
        install_context.add_class ("green");

        install_size_label = new Gtk.Label (null);
        install_size_label.selectable = true;
        install_size_label.max_width_chars = 50;
        install_size_label.wrap = true;
        install_size_label.xalign = 0;
        ...
        details_grid.attach (install_size_icon, 0, 0);
        details_grid.attach (install_size_label, 1, 0);
    ...
    public void display_details (string size, bool extra_repo) {
        install_size_label.label = _("Install size may be up to %s").printf (size);
